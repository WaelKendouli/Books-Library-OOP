...
public:
	...
	{
		_DrawScreenHeader("\tBorrow a Book Screen");
		string Name = _ReadBookName();
		clsBook  Book = clsBook::Find(Name);
		while (Book.IsEmpty() == true || Book.Status=="Borrowed")
		{
			cout << "Book was not found or already has been taken try again \n\n";
			Name = _ReadBookName();
