...
#include <iomanip>
using namespace std;
class clsFindByBookNameScreen : protected clsScreen
{
private :
...
public :
	static void ShowFindBookByNameScreen()
	{
		_DrawScreenHeader("\tFind a Book by Name Screen");
		string Name = _ReadBookName();
		clsBook  Book = clsBook::Find(Name);
		while (Book.IsEmpty() == true)
		{
			cout << "Book was not found try again \n\n";
			Name = _ReadBookName();
